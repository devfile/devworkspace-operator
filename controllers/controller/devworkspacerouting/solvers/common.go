			...
			if endpoint.Attributes.GetBoolean(string(controllerv1alpha1.DiscoverableAttribute), nil) {
				...
				services = append(services, corev1.Service{
					ObjectMeta: metav1.ObjectMeta{
						...
						Annotations: serviceAnnotations(nil, true, routingSpec.Service[componentName]),
					},
...
func GetServiceForEndpoints(routingSpec controllerv1alpha1.DevWorkspaceRoutingSpec, meta DevWorkspaceMetadata, includeDiscoverable bool, exposureType ...controllerv1alpha1.EndpointExposure) *corev1.Service {
	...
	for componentName, es := range endpoints {
		...
		annotations = serviceAnnotations(annotations, false, routingSpec.Service[componentName])
	}
