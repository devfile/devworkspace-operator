...
func (r *PrunerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if config.Config.Workspace.CleanupCronJob == nil || *config.Config.Workspace.CleanupCronJob.Enable == false {
...
func (r *PrunerReconciler) ensureCronJob(ctx context.Context, namespace string, config *controllerv1alpha1.CleanupCronJobConfig) error {
	suspend := *config.Enable == false
	image := PrunerImage
	if config.Image != "" && config.Image != PrunerImage {
		image = config.Image
	}
	dryRun := false
	...
	if *config.RetainTime != 0 && *config.RetainTime != 2592000 {
		retainTime = strconv.FormatInt(int64(*config.RetainTime), 10)
	}
	configMapName := PrunerConfigMap
	if config.CronJobScript != "" && config.CronJobScript != PrunerConfigMap {
	...
	if err := r.Get(ctx, client.ObjectKey{Namespace: namespace, Name: PrunerCronJobName}, cronJob); err != nil {
		if apierrors.IsNotFound(err) {
			return r.Create(ctx, prunerCronJob(namespace, suspend, dryRun, image, retainTime, configMapName))
		}
		return err
	}
...
func prunerCronJob(namespace string, suspend, dryRun bool, image, retainTime, configMapName string) *batchv1.CronJob {
	labels := resourceLabels()
